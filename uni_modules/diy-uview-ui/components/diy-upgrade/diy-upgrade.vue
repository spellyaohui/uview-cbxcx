...
<script>
	...
	export default {
		...
		computed: {
			...
			isAppStore() {
				return (this.isiOS||this.isHarmony) || (!this.isiOS && !this.isWGT && this.url.indexOf('.apk') === -1)
			},
			isHarmony() {
						...
						plus.runtime.getProperty(plus.runtime.appid,async (wgtinfo) => {
							...
							if(res.url){
								...
								if(compare(thiz.version,wgtinfo.version)){
									...
									thiz.silent = res.silent == 1? true:false //是否静默更新
									thiz.is_mandatory = res.is_mandatory||res.is_mandatory==1 ? true:false
									if (thiz.silent && thiz.isWGT) {	//静默模式且是WGT包，则静默更新
										thiz.downloadPackage()
									} else if(!thiz.silent && thiz.isWGT){	//如果是WGT包，但不是静默模式，则停止执行
										return;
									} else {	//下载更新
										thiz.checkLocalStoragePackage()
									}
								}
							}
						});
											...
											entry.getParent((parent) => {
												...
												entry.copyTo(
													...
													(res) => {
														...
														// 强制更新或静默模式，直接安装
														if (this.is_mandatory||this.silent) {
															this.installPackage();
														}
													},
													failCallback
