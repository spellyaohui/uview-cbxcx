	...
	computed: {
		barStyle(){
			return {
				...
				lineHeight:(this.verifyHeight)+'px',
				height:(this.verifyHeight)+'px',
		...
		blockStyle(){
			...
			if(this.isSuccess){
				...
				style.left = this.moveLeft+'px'
			}
			if(this.buttonBgColor){
				style.backgroundColor = this.buttonBgColor
			}
			if(this.buttonColor){
		...
		trueMoveableW:function(){
			return this.barWidth - this.verifyHeight
		}
	},
	mounted() {
		let thiz = this;
