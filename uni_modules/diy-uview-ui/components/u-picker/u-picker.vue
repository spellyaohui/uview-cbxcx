...
export default {
	...
	methods: {
		...
		initTimeValue(prefix='') {
			...
			if(this.mode=='time'){
				if (this.params.year&&!this.params.month&&fdate) {
					 return fdate
				}else if (!this.params.year&&this.params.month&&!this.params.day&&fdate) {
					 return fdate
				}
			}
		...
		// 用户更改picker的列选项
		change(e) {
			if(e&&e.detail&&e.detail.value&&this.valueArr.length==e.detail.value.length){
